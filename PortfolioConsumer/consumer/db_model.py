...
app.config['SQLALCHEMY_DATABASE_URI'] = \
    f"mysql+pymysql://{get_config()['user']}:{get_config()['pwd']}@{get_config()['url']}:{get_config()['port']}/sauvignon" \
    f'?autocommit=true'
...
def session_scope():
    ...
    try:
        ...
        clogger.error(create_msg("consumer", f"got message : {e}"))
        session.rollback()
        raise
    finally:
        session.close()
...
class UserMobileVerifInfo(db.Model):
    ...
    user_phone_number = db.Column(db.String, nullable=True)
    lst_update_dtim = db.Column(db.String, nullable=True,
